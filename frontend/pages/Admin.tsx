...
import { ensureGujaratiTranslation } from '../components/PopupContext';
...
const Admin: React.FC = () => {
    ...
    useEffect(() => {
        const calculateActiveUsers = () => {
            const count = users.filter(u => u.registrationStatus === 'APPROVED').length;
    ...
    // Tabs
    const [activeTab, setActiveTab] = useState<'dashboard' | 'registrations' | 'scanner' | 'audit' | 'settings' | 'payments' | 'verification' | 'analytics' | 'reports' | 'content' | 'feedback' | 'inquiries' | 'contactUs' | 'tasks' | 'surnames'>('dashboard');
    ...
    // Edit Settings State
    const [editPrices, setEditPrices] = useState<Record<string, any>>(DEFAULT_PRICES);
    ...
    // Feedback, Inquiries & Contact Messages
    ...
    const fetchData = async () => {
        const [u, b, s, a, logs, f, i, cMsg, sList] = await Promise.all([
            mockDb.getUsers(),
            mockDb.getBookings(),
            mockDb.getSettings(),
        ...
        if (s) {
            ...
            setEditPassDownloadEnabled(!!s.passDownloadEnabled);
        }
    };
    ...
    const saveSystemSettings = async () => {
        ...
        const finalPrices = {
            MALE_PASS: Number(editPrices.MALE_PASS) || 0,
            FEMALE_PASS: Number(editPrices.FEMALE_PASS) || 0,
            TWO_WHEELER: Number(editPrices.TWO_WHEELER) || 0,
            FOUR_WHEELER: Number(editPrices.FOUR_WHEELER) || 0,
        };

        // Price changes
        if (finalPrices.MALE_PASS !== settings.prices.MALE_PASS) changes.push(`Male Pass: ${settings.prices.MALE_PASS}->${finalPrices.MALE_PASS}`);
        if (finalPrices.FEMALE_PASS !== settings.prices.FEMALE_PASS) changes.push(`Female Pass: ${settings.prices.FEMALE_PASS}->${finalPrices.FEMALE_PASS}`);
        if (finalPrices.TWO_WHEELER !== settings.prices.TWO_WHEELER) changes.push(`2W Parking: ${settings.prices.TWO_WHEELER}->${finalPrices.TWO_WHEELER}`);
        if (finalPrices.FOUR_WHEELER !== settings.prices.FOUR_WHEELER) changes.push(`4W Parking: ${settings.prices.FOUR_WHEELER}->${finalPrices.FOUR_WHEELER}`);

        const updatedSettings = await mockDb.updateSettings({
            prices: finalPrices,
        ...
        setEditPrices(updatedSettings.prices);
    ...
    const handleBroadcast = async () => {
        ...
        // Auto-translate announcement into Gujarati script
        const finalGu = ensureGujaratiTranslation(announcement);
        ...
        try {
            const bc = new BroadcastChannel('svar_announcements_channel');
            bc.postMessage({
                type: 'ANNOUNCEMENT_BROADCAST',
                announcement: announcement,
                announcementGu: finalGu,
                parkingFull: settings?.parkingFull || false
            });
            bc.close();
        } catch (e) {}
        ...
        try {
            ...
            for (const user of usersWithEmail) {
                await mockDb.sendEmail(
                    user.email!,
                    "SVAR 2026 - Important Announcement / અગત્યની ઇવેન્ટ જાહેરાત",
                  ...
                  <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; overflow: hidden;">
                      ...
                      <div style="padding: 30px; line-height: 1.6;">
                          ...
                          <div style="background: #fff5f5; border-left: 4px solid #731515; padding: 15px; margin: 15px 0;">
                              <strong>English:</strong> ${announcement}
                          </div>
                          <div style="background: #eff6ff; border-left: 4px solid #1d4ed8; padding: 15px; margin: 15px 0;">
                              <strong>ગુજરાતી:</strong> ${finalGu}
                          </div>
                          <br/>
      ...
      */}
            ...
            <div className="flex flex-col xl:flex-row justify-between items-center mb-10 border-b border-viren-200 pb-6 gap-6 xl:gap-0">
                ...
                <div className="flex flex-wrap justify-center gap-2">
                    <div className="flex flex-wrap justify-center bg-white rounded-lg border border-viren-200 p-1 gap-1">
                        ...
                        <button onClick={() => setActiveTab('contactUs')} className={`px-3 py-2 rounded text-sm font-bold flex items-center gap-2 ${activeTab === 'contactUs' ? 'bg-viren-950 text-white' : 'text-viren-600 hover:bg-viren-50'}`}><Phone size={16} /> Contact Us</button>
                            ...
                            <div className="bg-white border border-viren-200 rounded-lg p-6 shadow-sm">
                                ...
                                <div className="space-y-4">
                                    <textarea
                                        ...
                                        placeholder="Type event announcement or emergency update..."
                                    ...
                                    <p className="text-[10px] text-viren-500 text-center italic">
                                        * Broadcaster automatically converts your message into Gujarati and publishes popups & emails in both languages.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input 
                                        type="number" 
                                        value={editPrices.MALE_PASS ?? ''} 
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            setEditPrices({ ...editPrices, MALE_PASS: val === '' ? '' : (parseInt(val, 10) || 0) });
                                        }} 
                                        className="w-full p-2 border border-viren-200 rounded text-sm bg-viren-50 font-bold" 
                                    />
                                </div>
                                <div>
                                    ...
                                    <input 
                                        type="number" 
                                        value={editPrices.FEMALE_PASS ?? ''} 
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            setEditPrices({ ...editPrices, FEMALE_PASS: val === '' ? '' : (parseInt(val, 10) || 0) });
                                        }} 
                                        className="w-full p-2 border border-viren-200 rounded text-sm bg-viren-50 font-bold" 
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-2">
                                <div>
                                    ...
                                    <input 
                                        type="number" 
                                        value={editPrices.TWO_WHEELER ?? ''} 
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            setEditPrices({ ...editPrices, TWO_WHEELER: val === '' ? '' : (parseInt(val, 10) || 0) });
                                        }} 
                                        className="w-full p-2 border border-viren-200 rounded text-sm bg-viren-50 font-bold" 
                                    />
                                </div>
                                <div>
                                    ...
                                    <input 
                                        type="number" 
                                        value={editPrices.FOUR_WHEELER ?? ''} 
                                        onChange={(e) => {
                                            const val = e.target.value;
                                            setEditPrices({ ...editPrices, FOUR_WHEELER: val === '' ? '' : (parseInt(val, 10) || 0) });
                                        }} 
                                        className="w-full p-2 border border-viren-200 rounded text-sm bg-viren-50 font-bold" 
                                    />
                                </div>
            ...
            {activeTab === 'inquiries' && (
                <div className="bg-white border border-viren-200 rounded-lg shadow-xl animate-fade-in overflow-hidden">
                    <div className="p-6 border-b border-viren-100 flex justify-between items-center bg-viren-50">
                        <div>
                            <h3 className="text-xl font-bold text-viren-950">Sponsorship & General Inquiries</h3>
                            <p className="text-sm text-viren-600">Inquiry submissions from Sponsorship and partnership forms</p>
                        </div>
            ...
            {activeTab === 'contactUs' && (
                <div className="bg-white border border-viren-200 rounded-lg shadow-xl animate-fade-in overflow-hidden">
                    <div className="p-6 border-b border-viren-100 flex justify-between items-center bg-viren-50">
                        <div>
                            <h3 className="text-xl font-bold text-viren-950">Contact Us Messages</h3>
                            <p className="text-sm text-viren-600">Direct user messages and support requests from the Contact Us page</p>
                        </div>
                        <div className="bg-viren-red text-white px-4 py-1 rounded-full text-xs font-bold">
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-viren-100">
                                {contactMessages.length === 0 ? (
                                    <tr>
                                        <td colSpan={3} className="p-8 text-center text-viren-400 italic">No Contact Us messages received yet.</td>
                                    </tr>
                                ) : (
                                    contactMessages.map((c, idx) => (
                                        <tr key={idx} className="hover:bg-viren-50 transition-colors">
                                            <td className="p-4 text-xs text-viren-500 whitespace-nowrap">{new Date(c.timestamp).toLocaleString()}</td>
                                            <td className="p-4">
                                                <p className="font-bold text-sm text-viren-950">{c.name}</p>
                                                <p className="text-xs text-viren-600">{c.phone}</p>
                                                {c.email && <p className="text-[10px] text-viren-500">{c.email}</p>}
                                            </td>
                                            <td className="p-4 text-sm text-viren-700 leading-relaxed whitespace-pre-wrap">
                                                {c.message}
                                            </td>
                                        </tr>
                                    ))
