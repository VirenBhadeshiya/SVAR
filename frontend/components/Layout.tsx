...
import { PopupProvider, ensureGujaratiTranslation } from './PopupContext';
import { PopupModal } from './PopupModal';
import { AIChatbot } from './AIChatbot';

const MarqueeAnnouncement: React.FC = () => {
  const [announcement, setAnnouncement] = useState<{ en: string; gu: string } | null>(null);
  ...
  useEffect(() => {
    const checkAnnouncement = async () => {
      ...
      if (settings.lastAnnouncement) {
        setAnnouncement({
          en: settings.lastAnnouncement,
          gu: settings.lastAnnouncementGu || ensureGujaratiTranslation(settings.lastAnnouncement)
        });
      } else {
    ...
    checkAnnouncement();
    const interval = setInterval(checkAnnouncement, 1500);
  ...
  return (
    <div className="bg-viren-red text-white py-1.5 overflow-hidden relative z-[60] border-b border-white/10 shadow-inner">
      <div className="flex whitespace-nowrap animate-marquee items-center">
        {[...Array(10)].map((_, i) => (
          <div key={i} className="flex items-center gap-4 px-4 font-bold text-[10px] md:text-xs uppercase tracking-[0.15em]">
            <Bell size={14} className="text-white/80" />
            <span>{announcement.en}</span>
            <span className="text-amber-300 font-sans tracking-normal font-medium">({announcement.gu})</span>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <a href="https://maps.google.com" target="_blank" rel="noreferrer" className="text-viren-redlight text-xs mt-2 inline-block hover:underline">View on Map</a>
        </div>
        <div>
