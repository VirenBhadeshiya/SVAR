...
const STORAGE_KEY_CONTACT_MESSAGES = 'svar_db_contact_messages';
...
export const mockDb = {
  ...
  getSettings: async (): Promise<SystemSettings> => {
    try {
        ...
        if (res.ok) {
            ...
            if (apiSettings && typeof apiSettings === 'object' && apiSettings.prices) {
                const merged: SystemSettings = {
                    parkingFull: apiSettings.parkingFull ?? settings.parkingFull,
                    lastAnnouncement: apiSettings.lastAnnouncement ?? settings.lastAnnouncement,
                    lastAnnouncementTime: apiSettings.lastAnnouncementTime ?? settings.lastAnnouncementTime,
                    registrationOpen: apiSettings.registrationOpen ?? settings.registrationOpen,
                    eventStartDate: apiSettings.eventStartDate ?? settings.eventStartDate,
                    eventEndDate: apiSettings.eventEndDate ?? settings.eventEndDate,
                    prices: { ...DEFAULT_PRICES, ...(apiSettings.prices || {}) },
                    passDownloadEnabled: apiSettings.passDownloadEnabled ?? settings.passDownloadEnabled
                };
                settings = merged;
        ...
        console.warn("Backend settings fetch failed, using local storage fallback", e);
    }
  ...
  updateSettings: async (newSettings: Partial<SystemSettings>): Promise<SystemSettings> => {
      ...
      settings = { ...settings, ...newSettings, prices: mergedPrices };
      saveToStorage(STORAGE_KEY_SETTINGS, settings);
      ...
      try {
          ...
          console.warn("Server settings sync failed, saved locally", e);
      }
  ...
  saveContactMessage: async (contact: { name: string, phone: string, email: string, message: string }): Promise<void> => {
      return new Promise(resolve => {
          const messages = loadFromStorage<any[]>(STORAGE_KEY_CONTACT_MESSAGES, []);
          messages.unshift({
              id: Date.now().toString(),
              ...contact,
              timestamp: new Date().toISOString()
          });
          saveToStorage(STORAGE_KEY_CONTACT_MESSAGES, messages);
          resolve();
      });
  },
  ...
  getContactMessages: async (): Promise<any[]> => {
      return new Promise(resolve => {
          resolve(loadFromStorage<any[]>(STORAGE_KEY_CONTACT_MESSAGES, []));
      });
  },
